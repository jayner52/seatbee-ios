...
| HIGH | Print exports — name cards, table cards, seating chart catalogue | Web's `PrintExport.jsx` ships ~20 themed designs (Classic Engraved, Modern Minimal, Romantic Script, Rustic Kraft, Botanical Garden, Art Deco Glam, Editorial Bold, Boho Earthy, Retro Marquee, Blush Pink, Mid-Century, Watercolor Wash, Bubblegum Bloom, Garden Ribbon, Candlelight Tent, Floral Frame, Whimsical Bow, Black Satin Bow, Citrus Welcome, Crimson Lace, Blue Toile) rendered as React components with complex SVG art, custom Google Fonts, and `html2canvas` / `window.print()` capture. iOS currently has a minimal native `PDFExportService` whose output is significantly worse. The right fix is a hosted print route on web (`/print/<exportType>?planId=…&theme=…`) that renders just the print HTML; iOS loads it in `WKWebView` and captures via `WKWebView.createPDF()`, with watermark applied server-side based on tier. Until then, the iOS Export tab should surface a "Visit seatbee.app for the full place card / chart / table label catalogue" hint so users know where the polished designs live. |
