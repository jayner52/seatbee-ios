...
User data in production was never at risk — RLS scopes iOS to the authenticated user's own plans — but the round-trip safety story now extends past the dev sandbox: every iOS App Store user has the same lossless round-trip.
...
- iOS is live on the App Store (corrected 2026-05-28 — prior wording in this file was stale)
...
`tier` (string: `free` | `pro` | `event_pass` | `signature_pass` | `pro_pass`), `event_pass_expires_at` (ISO timestamp), `event_pass_purchased_at` (ISO timestamp).

As of 2026-05-28 (subscription migration): `pro` is the canonical paid tier going forward. The three legacy values (`event_pass`, `signature_pass`, `pro_pass`) remain in the enum so existing rows still decode without error during the migration window — iOS preserves any raw value it sees and never writes the column.
...
### User-level subscription columns (top-level on `profiles` row, post-2026-05-28)

`subscription_status` (string: `active` | `past_due` | `canceled` | `none`), `subscription_renews_at` (ISO timestamp), `subscription_provider` (string: `stripe` | `apple` | `grace`), `subscription_id` (string — Stripe sub ID or Apple originalTransactionId).

These are the source of truth for Pro entitlement going forward. Web's Stripe webhook writes them on subscription lifecycle events; iOS's `/api/apple-iap` handler writes them on App Store transactions. **iOS never writes these columns** — same contract as the plan-level pass columns above.
...
| 2026-05-28 | Pricing migration: collapsed three one-time pass tiers ($29/$49/$149) into a single $9.99/mo auto-renewable subscription (`pro`). Free tier guest cap tightened 100 → 50 to create real conversion pressure. New shared columns on `profiles`: `subscription_status` / `subscription_renews_at` / `subscription_provider` / `subscription_id`. | iOS PR (`claude/pricing-subscription-analysis-HP8Ir`) — added `.pro` to `PlanTier` (legacy cases retained for round-trip), `.proMonthly` to `SeatbeeProduct`, `UserSubscription` model in `Models/TierLimits.swift`, `DatabaseService.fetchUserSubscription()` with quiet fallthrough for pre-migration backend, `AppState.refreshSubscription()` + subscription-first resolution in `activePlanTier`, single-card `UpgradeView` with Manage Subscription deep-link, `StoreKitService.hasActiveSubscription` driven by `Transaction.currentEntitlements`. `PlanTier.from()` now logs loudly on unknown raw values per the cardinal rule (no silent fallback). Web counterpart (Phase 1 of migration plan) lives in `jayner52/Seated` — must ship Stripe webhook + Supabase column migration + 30-day grace backfill before flipping. |
...
| LOW | Onboarding: CSV column mapping UI | Web onboarding has a column-mapping editor that auto-detects CSV headers (Name / Email / Side / Dietary / Party / Meal / etc.) and lets the user reassign columns. iOS uses the shared `GuestCSVParser` (auto-detects platform: Joy / Zola / The Knot / Eventbrite / Generic) but doesn't expose a manual remap UI. Acceptable trade-off; defer manual mapping to a future "Power Import" feature. |
| LOW | Onboarding: CSV template downloads | Web shows three "Download template" buttons (Wedding / Corporate / Generic) that emit pre-headed `.csv` files. iOS doesn't yet. Easy add — generate the same headers in-code and write to a temp file with `ShareLink`. Defer until users ask. |
| LOW | Onboarding: 3-phase floor-plan trace (Trace → Dimensions → Label zones) | Web onboarding lets the user upload a floor plan image and trace a custom polygon inline. iOS Step 3 now accepts photo / PDF uploads and stores them as `rawFloorPlanImage`, but the polygon trace + zone-labelling UX still happens in canvas `RoomSetupSheet` after creation. Acceptable for now — surface a "Trace floor plan now?" inline option on the onboarding screen later if users ask. |
| LOW | Onboarding: Pass selection / pre-purchase upsell | Web's room-size step shows a contextual pass-tier upsell (Event Pass for medium / Grand Pass for ballroom+). iOS skips this — defer pass purchase to in-app Settings → Event Passes after creation. |
...
| RESOLVED-VIA-WEB | Print exports — name cards, table cards, seating chart catalogue | Web's `PrintExport.jsx` ships ~20 themed designs rendered as React components with complex SVG art, custom Google Fonts, and `html2canvas` / `window.print()` capture. **Decision (2026-05-08): defer this entire surface to web.** Building a parallel native iOS print pipeline (or even a WKWebView capture pipeline) doesn't earn its keep — the design space is large, templates change often, and iOS users can hand off to web in one tap. The iOS "Place Cards (PDF)" card has been replaced with **"Printable Cards & Charts (opens on web)"** which deep-links into the canonical Print & Design modal. Native PDFs that ARE in scope on iOS (Seating Chart PDF, Planner View PDF) stay native — those are functional plan documents, not designed print artefacts. **Deep-link wire format (implemented 2026-05-08):** iOS opens `https://seatbee.app/app?planId=<id>&openExport=printCards`. Web's `AppContent` captures both params on mount, stashes them as `localStorage.seatbee_deep_link = {planId, openExport, ts}` (so the intent survives OAuth / sign-in redirects), and consumes once `user` is truthy by calling `loadPlan(planId)` then `setShowPrintExport(true)`. The print modal is mounted globally as `<GlobalPrintExportModal/>` so it can pop without ExpPanel being on screen. Payloads expire after 30 min so a forgotten one doesn't trigger months later. |
