...
| MEDIUM | Onboarding: Floor plan upload + 3-phase RoomEditorModal | Web onboarding (Step 4 + 5) lets the user upload a floor plan image and trace a custom polygon in 3 phases (Trace → Dimensions → Label zones). iOS has the same UI in `Features/Editor/RoomSetupSheet` but it's only accessible from the canvas after the plan is created. To match: surface RoomSetupSheet as an optional Step 3.5 of onboarding, OR present it inline on the Room Setup step when the user picks "Custom" shape. |
...
| LOW | Onboarding: URL pre-fill (placeId, venueName) | Web reads `?placeId=` and `?venueName=` URL params on load to pre-populate the venue field for users coming from venue-discovery referrals. iOS doesn't accept URL params during onboarding. Add a deep-link handler when iOS gets a venue-discovery surface. |
