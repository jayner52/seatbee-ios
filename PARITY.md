...
| 2026-05-04 | Web auto-sync overwrote iOS-canonical rule types with legacy `seatTogether` strings (state hydrated from old DB rows pre-iOS-migration; auto-sync wrote them back, fighting iOS) | Web direct-to-main commit `4328234` (rule-type translator at all 3 web load paths so state is always canonical) + iOS PR #5 (legacy→canonical mapping in `RuleType.parse`) |
